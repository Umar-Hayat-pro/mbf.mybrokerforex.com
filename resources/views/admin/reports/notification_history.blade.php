...
<script>
    $('.notifyDetail').click(function(){
        ...
        if($(this).data('type') == 'email'){
            var message = `<iframe id="emailDetailIframe" src="${message}" style="width:100%; height:500px; border:none;" title="Iframe Example"></iframe>`;
        }
        $('.detail').html(message);
        ...
        $('#emailDetailIframe').on('load', function() {
            var iframe = document.getElementById('emailDetailIframe');
            var iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
            
            // Change the image inside the iframe
            var img = iframeDocument.querySelector('img'); // Adjust the selector as needed
            if (img) {
