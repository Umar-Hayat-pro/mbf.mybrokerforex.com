                ...
                <tbody>
                    @foreach ($ads as $ad)
                        <tr>
                            ...
                            <td>
                                <div>
                                    {{ showAmount(@$ad->price) }} {{ @$ad->fiat->symbol }} / {{ __(@$ad->asset->symbol) }}
                                    <br>
                            ...
                            <td>
                                <div>
                                    <span class="d-block">
                                        {{ showAmount(@$ad->minimum_amount) }} - {{ showAmount(@$ad->maximum_amount) }}
                                        {{ @$ad->fiat->symbol }}
                                    </span>
                                    ...
                                    @else
                                        ...
                                        @if ($ad->complete_step != 3)
                                            <i class="fas fa-info-circle text--danger cursor-pointer error-message"
                                                data-message="@lang("You didn't provide all the required data to publish an ad.")"></i>
                                        @else
                                            <i class="fas fa-info-circle text--danger cursor-pointer error-message"
                                                data-message="@lang('You don\'t have sufficient wallet balance to publish this ad.')"></i>
                                        @endif
                                    @endif
                                </div>
                            <td>
                                <div class="text-end">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a href="{{ route('user.p2p.advertisement.create', $ad->id) }}?step=1"
                                                class="text--success dropdown-item">
                                                <i class="las la-edit"></i> @lang('Edit')
                                            </a>
                                            <button class="text--base detailsBtn dropdown-item"
                                                data-ad='@json($ad)'>
                                                <i class="la la-desktop"></i> @lang('Details')
                                            </button>
                                            @if ($ad->status == Status::DISABLE)
                                                <button class="text--success confirmationBtn dropdown-item"
                                                    data-question="@lang('Are you sure to enable this ad')?"
                                                    data-action="{{ route('user.p2p.advertisement.change.status', $ad->id) }}">
                                                    <i class="la la-eye"></i> @lang('Enable')
                                                </button>
                                            @else
                                                <button class="text--danger confirmationBtn dropdown-item"
                                                    data-question="@lang('Are you sure to disable this ad')?"
                                                    data-action="{{ route('user.p2p.advertisement.change.status', $ad->id) }}">
