                ...
                <form class="d-flex gap-2 flex-wrap">
                    <div class="flex-fill">
                        <div class="input-group">
                            <select name="order_type" class="form-control form--control2 submit-form-on-change form-select">
                                ...
                                <option value="{{ Status::ORDER_TYPE_LIMIT }}"
                                    @selected(request()->order_type == Status::ORDER_TYPE_LIMIT)>
                                    @lang('Limit')
                                </option>
                                <option value="{{ Status::ORDER_TYPE_MARKET }}"
                                    @selected(request()->order_type == Status::ORDER_TYPE_MARKET)>
                                    @lang('Market')
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="flex-fill">
                        <select class="form-control form--control2 submit-form-on-change form-select" name="order_side">
                            ...
                            <option value="">@lang('All')</option>
                            <option value="{{ Status::BUY_SIDE_ORDER }}"
                                @selected(request()->order_side == Status::BUY_SIDE_ORDER)>
                                @lang('Buy')
                            </option>
                            <option value="{{ Status::SELL_SIDE_ORDER }}"
                                @selected(request()->order_side == Status::SELL_SIDE_ORDER)>
                                @lang('Sell')
                            </option>
                        </select>
                    </div>
                        ...
                        @forelse($orders as $order)
                                            <tr class="tr-{{ $order->id}}">
                                                ...
                                                <td>
                                                    <div class="order--amount-rate-wrapper">
                                                        <span class="order--amount d-block">
                                                            <span class="order--amount-value">{{ showAmount($order->amount) }}</span>
                                                            @if ($order->status == Status::ORDER_OPEN)
                                                                <span class="amount-rate-update"
                                                                    data-order='@json($order->only('amount', 'rate', 'id'))'
                                                        ...
                                                        <span class="order--rate d-block">
                                                            <span class="order--rate-value">{{ showAmount($order->rate) }}</span>
                                                            @if ($order->status == Status::ORDER_OPEN)
                                                                <span class="amount-rate-update"
                                                                    data-order='@json($order->only('amount', 'rate', 'id'))'
                                                ...
                                                <td>
                                                    <div>
                                                        <span class="d-block">
                                                            {{ showAmount($order->charge) }} {{ __($marketCurrencySymbol)}}
                                                        </span>
                                                        <span>
                                                            {{ showAmount($order->total) }} {{ __($marketCurrencySymbol) }}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td>
                                                                    ...
                                                                    @if ($order->status == Status::ORDER_OPEN)
                                                                                        @php
                                                                                            ...
                                                                                            if ($order->order_side == Status::BUY_SIDE_ORDER) {
                                                                                                ...
                                                                                                $cancelMessage .= showAmount($amount) . "+" . showAmount($chargeBackAmount) . "=" . showAmount($amount + $chargeBackAmount) . __(" $symbol to your $symbol wallet");
                                                                                            } else {
