...
<head>
    ...
    <link rel="stylesheet" href="{{ asset($activeTemplateTrue . 'css/color.php') }}?color={{ $general->base_color }}" rel="stylesheet">
</head>
...
<body>
    ...
    <script>
        window.my_pusher = {
            'app_key': "{{ base64_encode(@$general->pusher_config->pusher_app_key) }}",
            'app_cluster': "{{ base64_encode(@$general->pusher_config->pusher_app_cluster) }}",
            'base_url': "{{ route('home') }}"
        }
        ...
        (function($) {
            ...
            @if (!request()->routeIs('trade'))
                ...
                if (currentTheme) {
                    ...
                    let defaultTheme = `{{ $general->default_theme }}`;
