...
class TradeController extends Controller
{
    ...
    private function sellerFavour($trade)
    {
        ...
        if ($trade->charge > 0) {
            $details = "Return P2P sell order charge" . showAmount($trade->charge) . " " . @$trade->ad->fiat->symbol;
    ...
    {
        $buyer   = $trade->buyer;
        $details = "Realse(by admin) p2p buy order: " . showAmount($trade->asset_amount) . " " . @$trade->ad->fiat->symbol;
    ...
    public function messageSave(Request $request, $id)
    {
        ...
        $userHtml = view($this->activeTemplate . 'user.p2p.trade.single_message', ['message' => $message])->render();
