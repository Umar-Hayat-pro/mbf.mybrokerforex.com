...
class TradeController extends Controller
{
    public function request(Request $request, $id)
    {
        ...
        $html              = view($this->activeTemplate . "p2p.trade.request.$type", compact('pageTitle', 'ad', 'coinWalletBalance', 'type', 'feedback'))->render();
        ...
        return view($this->activeTemplate . 'p2p.trade', compact('pageTitle', 'ad'));
    }
    ...
    public function requestSave(Request $request, $id)
    {
        ...
        if ($request->fiat_amount < $ad->minimum_amount) {
            return jsonResponse("The minimum amount is " . showAmount($ad->minimum_amount) . " " . $ad->fiat->symbol);
        }

        if ($request->fiat_amount > $ad->maximum_amount) {
            return jsonResponse("The maximum amount is " . showAmount($ad->maximum_amount) . " " . $ad->fiat->symbol);
        }
        ...
        if ($type == 'sell') {
            ...
            if (!$userPaymentMethodExists) {
                ...
                $paymentMethodHtml = view($this->activeTemplate . "p2p.trade.add_payment_method", compact('paymentMethod'))->render();
        ...
        $trx     = getTrx();
        $details = "P2P Sell Order: " . showAmount($request->asset_amount) . " " . $ad->asset->symbol;
        $this->createTrx("-", $sellerWallet, $request->asset_amount, "p2p_sell_order", $details, $trx);

        $chargeAmount = ($request->asset_amount / 100) * gs('p2p_trade_charge');

        if ($chargeAmount > 0) {
            $details = "P2P Sell Order Charge: " . showAmount($chargeAmount) . " " . $ad->asset->symbol;
        ...
        notify($ad->user, 'P2P_TRADE', [
            'order_id'     => $trx,
            'asset_amount' => showAmount($trade->asset_amount),
            'fiat_amount'  => showAmount($trade->fiat_amount),
    ...
    public function details($id)
    {
        ...
        return view($this->activeTemplate . 'user.p2p.trade.details', compact('trade', 'pageTitle', 'user', 'sellerPaymentMethod', 'trader', 'messages', 'feedback', 'tradeFeedback', 'feedBackAbility', 'paymentTimeRemind', 'paymentTimeRemindInSecond'));
    }
    ...
    public function list($scope)
    {
        ...
        return view($this->activeTemplate . 'user.p2p.trade.index', compact('trades', 'pageTitle', 'user'));
    }
    ...
    public function cancel($id)
    {
        ...
        $seller  = $trade->seller;
        $details = "Cancel p2p sell order: " . showAmount($trade->asset_amount) . " " . @$trade->ad->asset->symbol;
        ...
        if ($trade->charge > 0) {
            $details = "Returned P2P sell order charge " . showAmount($trade->charge) . " " . @$trade->ad->asset->symbol;
    ...
    public function dispute($id)
    {
        ...
        notify(@$trade->ad->user, 'P2P_TRADE_REPORT', [
            ...
            'asset_amount' => showAmount($trade->ad->asset_amount),
            'fiat_amount'  => showAmount($trade->fiat_amount),
            'asset'        => @$trade->ad->asset->symbol,
            'fiat'         => @$trade->ad->fiat->symbol,
            'date'         => showDateTime($trade->ad->created_at),
    ...
    public function release($id)
    {
        ...
        $buyer   = $trade->buyer;
        $details = "Realse p2p buy order: " . showAmount($trade->asset_amount) . " " . @$trade->ad->fiat->symbol;
