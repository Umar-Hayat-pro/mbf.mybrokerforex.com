...
class OrderController extends Controller
{
    ...
    public function save(Request $request, $symbol)
    {
        ...
        if ($request->order_side == Status::BUY_SIDE_ORDER) {
            ...
            if ($amount < $pair->minimum_buy_amount) {
                $notify[] = "Minimum buy amount " . showAmount($pair->minimum_buy_amount) . ' ' . $coin->symbol;
                return response()->json([
                    'remark'  => 'validation_error',
                    'status'  => 'error',
                    'message' => ['error' => $notify],
            ...
            if ($amount > $pair->maximum_buy_amount && $pair->maximum_buy_amount != -1) { //-1 for unlimited maximum amount
                $notify[] = "Maximum buy amount " . showAmount($pair->maximum_buy_amount) . ' ' . $coin->symbol;
                return response()->json([
                    'remark'  => 'validation_error',
                    'status'  => 'error',
                    'message' => ['error' => $notify],
        ...
        if ($request->order_side == Status::SELL_SIDE_ORDER) {
            ...
            if ($request->amount < $pair->minimum_sell_amount) {
                $notify[] = "Minimum sell amount " . showAmount($pair->minimum_sell_amount) . ' ' . $coin->symbol;
                return response()->json([
                    'remark'  => 'validation_error',
                    'status'  => 'error',
                    'message' => ['error' => $notify],
            ...
            if ($request->amount > $pair->maximum_sell_amount && $pair->maximum_sell_amount != -1) {
                $notify[] = "Maximum sell amount " . showAmount($pair->maximum_sell_amount) . ' ' . $coin->symbol;
                return response()->json([
                    'remark'  => 'validation_error',
                    'status'  => 'error',
                    'message' => ['error' => $notify],
    ...
    private function updateAmount($request, $order, $user)
    {
        ...
        if ($request->amount <= $order->filled_amount) {
            $notify[] = "Already filled amount" . showAmount($order->filled_amount);
            return response()->json([
                'remark'  => 'validation_error',
                'status'  => 'error',
                'message' => ['error' => $notify],
        ...
        if ($request->amount < $minAmount) {
            $notify[] = "Minimum $type amount " . showAmount($minAmount) . ' ' . $currency->symbol;
            return response()->json([
                'remark'  => 'validation_error',
                'status'  => 'error',
                'message' => ['error' => $notify],
        ...
        if ($request->amount > $maxAmount && $maxAmount != -1) {
            $notify[] = "Maximum $type amount " . showAmount($maxAmount) . ' ' . $currency->symbol;
            return response()->json([
                'remark'  => 'validation_error',
                'status'  => 'error',
                'message' => ['error' => $notify],
