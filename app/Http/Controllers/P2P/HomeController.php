...
class HomeController extends Controller
{
    public function p2p($type = "buy", $coin = null, $currency = 'all', $paymentMethod = 'all', $region = 'all', $amount = 0)
    {
        ...
        if (request()->ajax()) {
            $html = view($this->activeTemplate . "p2p.ad.list", compact('ads', 'type'))->render();
        ...
        $sections       = Page::where('tempname', $this->activeTemplate)->where('slug', 'p2p')->first();
        $paymentMethods = PaymentMethod::select('supported_currency', 'id', 'slug', 'name', 'branding_color')->active()->get();

        return view($this->activeTemplate . 'p2p.trade.index', compact('pageTitle', 'coins', 'ads', 'sections', 'type', 'countries', 'currencies', 'paymentMethods', 'requestedCurrencyPaymentMethods', 'totalAds'));
    }
    ...
    public function advertiser($id)
    {
        ...
        $paymentMethods = PaymentMethod::select('supported_currency', 'id', 'slug', 'name', 'branding_color')->active()->get();
        return view($this->activeTemplate . 'p2p.advertiser_details', compact('pageTitle', 'advertiser', 'feedback', 'widget', 'ads', 'paymentMethods'));
    }
}
