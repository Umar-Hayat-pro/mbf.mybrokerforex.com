    ...
    public function buyPlaceHolder(): Attribute
    {
        return new Attribute(function () {
            if ($this->maximum_buy_amount <= 0) {
                return trans('Minimum ') . showAmount($this->minimum_buy_amount);
            } else {
                return showAmount($this->minimum_buy_amount) . '-' . showAmount($this->maximum_buy_amount);
            }
        });
    }
    ...
    public function sellPlaceHolder(): Attribute
    {
        return new Attribute(function () {
            if ($this->maximum_sell_amount <= 0) {
                return trans('Minimum ') . showAmount($this->minimum_sell_amount);
            } else {
                return showAmount($this->minimum_sell_amount) . '-' . showAmount($this->maximum_sell_amount);
            }
        });
    }
